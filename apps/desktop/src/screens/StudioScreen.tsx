...
export function StudioScreen() {
  ...
  const generate = () => {
    setError(null);
    void runMusicGeneration({ request, generate: generateTrack, preview: previewDraft, onPhase: setPhase }).catch((reason: unknown) => {
      setPhase("failed");
      setError(reason instanceof MusicGenerationPipelineError && reason.stage === "audio" ? previewErrorMessage() : generationErrorMessage());
    });
  };
  ...
  return (
    <Screen>
      ...
      {draft ? <Card className="selected-card"><span className="label">生成したBGM</span><h2>{draft.title}</h2><p>{draft.description}</p><div className="row"><span className="pill">{draft.bpm} BPM</span><span className="pill">余韻 {draft.tailSeconds}秒</span></div><div className="row"><Button label={phase === "audio" ? "音声を生成・検証中…" : draftPlaying ? "■ 停止" : draft.audioValidation === "passed" ? "▶ 再生" : "検証して再生"} variant="secondary" disabled={generating} onClick={() => { setError(null); if (draftPlaying) { void stopMusic().catch(() => setError(previewErrorMessage())); return; } setPhase("audio"); void previewDraft(draft).then(() => setPhase("completed")).catch(() => { setPhase("failed"); setError(previewErrorMessage()); }); }} /><Button label="ライブラリに保存" disabled={draft.audioValidation !== "passed"} onClick={() => void saveDraft()} /><Button label="破棄" variant="danger" disabled={generating} onClick={discardDraft} /></div>{draft.audioValidation === "deferred_until_focus_ends" ? <p className="warning">集中終了後に音声検証を再開できます。</p> : null}</Card> : null}
    </Screen>
  );
}
