...
function draft(audioValidation: MusicDraft["audioValidation"] = "required"): MusicDraft {
  return {
    ...
    bpm: 64, tailSeconds: 4, supercolliderSource: "(~lyraTrack = (synthDefs: [], pattern: Pseq([1], inf));)",
...
describe("BGM生成パイプライン", () => {
  test("コード生成後に音声を検証して完了する", async () => {
    ...
    const result = await runMusicGeneration({
      ...
      preview: async (target, onProgress) => {
  ...
  test("集中中は音声処理を呼ばず延期する", async () => {
    ...
    const result = await runMusicGeneration({ request, generate: async (_request, onProgress) => { onProgress({ phase: "coding" }); return draft("deferred_until_focus_ends"); }, preview: async (target) => { previewCalled = true; return target; }, onPhase: (phase) => phases.push(phase) });
  ...
  test("コード生成の失敗段階を保持する", async () => {
    await expect(runMusicGeneration({ request, generate: async () => { throw new Error("Codex stopped"); }, preview: async (target) => target, onPhase: () => undefined })).rejects.toMatchObject({ stage: "coding" });
  });

  test("音声処理の失敗段階を保持する", async () => {
    await expect(runMusicGeneration({ request, generate: async () => draft(), preview: async () => { throw new Error("scsynth stopped"); }, onPhase: () => undefined })).rejects.toMatchObject({ stage: "audio" });
  });
});
