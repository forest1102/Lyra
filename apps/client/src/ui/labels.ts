import type { MusicArrangement, MusicIntensity, MusicTheme, TimerPhase, TimerPreset } from "@lyra/domain";
...
const intensities: Record<MusicIntensity, string> = {
  low: "低",
  medium: "中",
  high: "高"
};
...
export function generationErrorMessage(): string {
  return "BGMの生成に失敗しました。Codex CLIのインストールと認証状態を確認してください。";
}

export function previewErrorMessage(): string {
  return "音声の検証・再生に失敗しました。SuperColliderの実行環境を確認してください。";
}
...
export function generationProgressLabel(phase: MusicGenerationPhase): string {
  switch (phase) {
    case "idle":
      return "";
    case "coding":
      return "1/2 SuperColliderをコーディング中…";
    case "audio":
      return "2/2 音声を生成・検証中…";
    case "deferred":
      return "コード生成完了。音声生成は集中終了後に再開できます。";
    case "completed":
      return "生成と音声検証が完了しました。試聴を再生しています。";
    case "failed":
      return "BGMの生成を完了できませんでした。";
  }
}
